...
module.exports = async function (img, detections, detection_options) {
    return new Promise(async function (resolve, reject) {
        try {
            // Check img is a canvas object or detections are valid or detection_options are valid
            if (!img instanceof Canvas || !detections.length || !isObject(detection_options)) reject();
            else {
                ...
                detections.forEach(face => {
                    try {
                        ...
                        const matched_face_label = (detection_options.recognise && bestMatch) ? [
                            `${bestMatch["_label"]} (${Math.round(bestMatch["_distance"] * 100, 2) / 100})`
                        ] : [];
