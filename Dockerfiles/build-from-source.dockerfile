# TF 
FROM tensorflow/tensorflow:2.3 AS tf


# # -------------------- Build Tensorflow --------------------
# # Specify a base image
# FROM debian:buster AS tf-stage
...
# RUN ./configure
# RUN bazel build --config=opt --config=monolithic //tensorflow/tools/lib_package:libtensorflow

# # -------------------- Build NPM packages --------------------
# # Specify a base image
# FROM debian:stretch AS npm-stage

# # Setup the build args
# ARG FACE_API_VERSION=0.21.0
# ARG TFJS_NODE_VERSION=latest
...
# COPY --from=npm-stage /usr/app/node_modules .
